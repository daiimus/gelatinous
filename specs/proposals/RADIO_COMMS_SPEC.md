...
> **Status:** 🚧 **PHASE 1 CORE SHIPPED (2026-07-04): player-facing radio is LIVE** — `world/radio.py` (single staff-locked channel + device-gated voice echo), `Radio` typeclass + `WALKIE_TALKIE` prototype, `transmit`/`xmit`/`xm` + `to <radio>,` retarget, `tune <device> to <freq|scan>`, `toggle <device> [on/off]`, state-aware look-readout (frequency shown only when powered). Voice-attributed (recognition on the air, modulator-defeatable), hearing-gated, every powered carried radio receives its band (scan catches all, freq-tagged). **WITNESS LINK + BOT TRANSCEIVERS ALSO SHIPPED (#1009):** the witness spawns with a real `WALKIE_TALKIE` on the **emergency band (911MHz)** and `witness_report` gates on it — snatch/break/KO = no report (a physical interdiction beside killing them; robbing them nets a walkie pre-tuned to the cop channel). Security bots carry a built-in **comms module** (ear/antenna augment organ, `factory_fit_comms`, like the riot gun) tuned to 911MHz — they hear the net via `comms_organ_frequency`; destroy/harvest the ear and the bot goes deaf (EMP/mute seam via medical hit-location). **ACQUISITION PLACED (2026-07-04):** the Colonial Armory stocks `WALKIE_TALKIE` at 0 tokens (live shop object) and ~1-in-3 gangers carry one (`chance_stock`) — Phase 1 is now player-obtainable end to end. **NPCs DOGFOOD THE REAL VERBS (2026-07-04):** the witness readies and calls it in through the actual player commands — `wield magpie` / `toggle magpie on` / `tune magpie to 911MHz` at spawn, `xmit <report>` + `emote` at report — never a backdoor `transmit()` call or direct `db` write. The NPC path is identical to the player path (level playing field; NPCs lead by example). Surfaced + fixed a real bug in the process: band matching is now **case-insensitive** (`world.radio.same_band`) so a band typed `911mhz` still reaches bots/constants stored as `911MHz`, while `tune` preserves the typed case for clean display **[STALE 2026-09-12: `tune` canonicalizes rather than preserving case — `CmdTune.func` writes `normalize_band(freq)`, so `911mhz` is stored and displayed as `911MHz`. Superseded by the megahertz-dial decision in §6.3 below.]**. **PHASE 2 RANGE — ✅ SHIPPED 2026-07-10 (#1128, same night as the sign-off):** `world/radio.py` range layer — constants RADIO_TX_RANGE=12 / MAST_TX_RANGE=999 **[now 20 — see the #1311 note later in this banner; world/radio.py:490]** / clear ≤0.7 / static ≤1.15 / +2 cells per z; `_deliver` grades every receiving set (clear/fuzzy/static/gone), fuzzy rides `_obscure_heard`, static = existence-only hiss (no speech payload, never elected), gone = silence; repeaters = powered on-band base stations w/ intact mast within origin reach, one hop, regenerate clean; off-grid fails open; **links are RECIPROCAL** (effective reach = max of both ends' antennas — the witness's handheld reaches dispatch because the CONSOLE's mast is tall; repeaters pick up anything their own ears cover; wrecking the mast deafens dispatch as well as muting it). The model as decided: range is wiring between machinery that already exists. (1) **Distance**: rooms carry live (x,y,z); `_deliver` measures speaker→receiver Chebyshev distance instead of assuming colony-wide reach. (2) **Falloff is RENDERED, not binary**: distance/`tx_range` ≤0.7 = crisp; 0.7–1.0 = clarity slides 1.0→0.2 through the EXISTING letter-drop comprehension renderer (the mutter machinery doubles as signal degradation — "c-ver the b-ck d--r" at the fringe of coverage); just past 1.0 = existence-only static wash; beyond = silence. (3) **The antenna is the RELAY**: handhelds/comms organs get modest `db.tx_range` (a few blocks); a base station with an INTACT mast is a one-hop repeater — traffic it hears on its band rebroadcasts at mast range (colony-wide). No relay chains. Consequence for free: wrecking the Constabulary mast collapses 911MHz from colony-wide to walkie-range clusters — dispatch goes deaf AND quiet beyond a few blocks. (4) **Elevation**: transmitter z adds range (rooftops matter, basements swallow signal). (5) **Fail-open**: coordinate-less rooms hear everything (no regression off-grid). Jamming = the inverse object later; battery orthogonal. Numbers (walkie cells-of-reach) = user's call at build time. **SHIPPED TOWARD IT (#1124): the dispatch transmission mast** — 'antenna mast' on the Constabulary roof, linked via `console.db.antenna`; wrecked (`db.intact` False) = no carrier at BOTH voice paths (acks + console answers); attack path SHIPPED (2026-07-11, #1164: `sabotage <structure>` / shared `repair` verb — channeled 90s/180s acts w/ visible tells, `db.breachable is True` gate on BOTH masts, wreck stashes/restores authored prose, completed sabotage = witnessed crime via report_crime; reciprocity makes it the heist — the wrecked mast can't carry the report of its own wrecking). **FIRST PUBLIC REPEATER (2026-07-11): the Queen of Cups house band** — tenant-rigged 'antenna mast' (#5081, `db.intact` seam) + 'repeater cabinet' (#5082, powered base station, `antenna`-linked) on the hotel roof (z=5), fixed on **88.8MHz** **[STALE 2026-09-12: `scripts/builds/018_qoc_raise.py` raised the Queen to z12 and moved #5081/#5082 to the new z12 roof; `019_fallen_antenna_bridge.py` / `020_fallen_antenna_gap.py` then toppled #5081 into "The Fallen Antenna" parkour crossing at (-3,-15,12). Coverage follows the object; neither is on a z=5 roof.]**; any handheld tuned there is heard by the mast's ears (reciprocity) and regenerated colony-wide — civilian counterpart to the 911MHz dispatch mast, scan-discoverable, sabotage-able the same way. **2026-07-25/28 ADDENDA:** THIRD REPEATER — the Brackett **AWE Sentinel-9** conformed to the two-object standard (mast #5641 structure w/ intact+breachable on the North Roof **[STALE 2026-09-12: `scripts/builds/011_brackett_crown.py` relocated #5641 off the roof to the new climbable antenna platform at the mast head, (-10,-18,19) — "coverage follows the object". Cabinet #6027 is unaffected.]**; head-end cabinet #6027 in the basement Utility Room, antenna-linked; a bespoke press-grammar breaker was built and REMOVED same day — infrastructure reuses standard verbs). Retuned to **88.8MHz = 'the Birdhouse'** (colony street name): two masts union the civilian band. **RADIO GEOGRAPHY (#1311):** MAST_TX_RANGE 999→20 and `_antenna_site()` at all four seams — a mast-linked station transmits/receives/relays FROM ITS STEEL (position+elevation), so coverage has real fringes and dead zones (911 goes thin in the deep southwest; far north is silent). **NPC RADIO (#1313/#1319):** LLM radio-mode replies air via real `xmit` (never room-say); `world/director/broadcasts.py` gives flagged NPCs scheduled on-air segments (the Rook, sealed in the Brackett basement, is the first station voice); venue **AWE Fireside-12 shelf radios** fan the band to whole rooms via the grille rule. Battery, jamming, encryption still open. Original scoping below.
>
...
| **Base station** | fixed room object | strong range; where an org's traffic converges (security base = the intel-sync point). **✅ FIRST BUILD (2026-07-10):** `BASE_STATION` console at the security base (heartbeat-installed, idempotent; **[STALE 2026-09-25 (#3667): the operator is NOT heartbeat-ensured any more — a dead dispatcher is a vacancy on the dispatch post, returned by her own sleeve policy or replaced by the sweep]**) — **dispatch's voice**: deterministic template acks on 911MHz for every event **[STALE 2026-09-12 (#1160): when units roll there is NO dispatch line — each responder acks in its own voice via `_unit_ack` ("Unit <id> responding — <where>.", staggered). The dispatch template survives only for the drained pool and is aired by the OPERATOR's `xmit` (`_transmit_ack`), never by the console.]** (`Dispatch copies — an assault at Cobb Street. Unit responding.` / `No units available.` when the pool drains — the finite pool made audible). Switch it off or wreck it and dispatch goes silent (the sabotage seam, live). No LLM for acks — and **the ANSWERING brain SHIPPED (2026-07-10)**: traffic that names dispatch ('dispatch/control/base') from a non-NPC speaker gets an answered line via the **civic LLM lane** **[STALE 2026-09-12: the dispatch REPLY is no longer a civic-lane line and there is no 'names dispatch' gate. `LLMNpcMixin._hear_radio` defers emergency-band traffic heard at a seated board to `world/souls/salience.py::_work_radio_traffic`, which judges the call and then calls `answer_traffic` → `_try_llm_reply(…, "radio")` — the NPC's ordinary GM lane. The civic lane now serves the structured report CLASSIFIER (`request_civic_verdict`, `world/director/radio_report.py`) and the combat reflex (`world/llm/reflex.py`). The gate is "seated at a powered 911MHz board".]** (`CIVIC_LLM_URL` — a second OpenAI-compatible endpoint; an on-device AFM shim serves it today at ~0.4s warm, but the game never learns Apple exists — swap by URL). **OPERATOR v1 (2026-07-10): the far end is a PERSON.** Vess — an embodied GM-lane NPC seated at the base (heartbeat-ensured) — is the voice of dispatch: acks and answers transmit AS HER (smoky rasp on the air, voice-recognisable/rememberable/modulator-defeatable), her register carried by operator-voiced exemplars in the civic instructions. Dead/unconscious/absent operator = **SILENCE** (updated 2026-08-23, #2223/#2228 — the automation voice is deleted, not disabled: the colony is operated by its people, and an unmanned emergency line is the setting rather than a hole in it). The console no longer speaks OR dispatches; whoever is seated at it judges the call and answers in their own voice through the real `xmit`, so nothing anywhere has to check whether the desk is empty — nobody is there to hear it. Three shifts staff it now (Petra/Kiro/Ines, #2233); the ack beat varies 1.5–3.5s (a human pause, not a trigger). Rotating shift cast = the agreed follow-up. **THE DESK SEAM (2026-07-10):** a base station is a TRANSMIT device for whoever is SEATED at it (`seated_base_station` — worn/held gear still wins; `xmit` low, `to <console>, <msg>` overt). Whoever holds the chair holds the desk: attribution stays honestly the sitter's voice, so a seized board broadcasts the seizer, not the operator — and a dead console keys nothing (sabotage gate intact). The operator herself lives in the **dispatch room** (`@patrol/dispatch` tag, 2026-07-10 — console+operator upkeep prefers it, security base = fallback + respawn point). Operator renamed **Petra**; dispatch now answers from the Constabulary's secure 2nd floor while units respawn at the Colonial Landing Pad. Template fallback is STRUCTURAL (model failure/refusal → canned line, never silence) **[STALE 2026-09-12, and contradicts "= SILENCE / the automation voice is deleted" earlier in this same row: failure is silence at every layer now (`_try_llm_reply` defaults `on_fail=_llm_silent`; `consider_radio_report`: "Silence on every failure"; `AnsweringFixture`: "Silence is the honest report"). The surviving canned lines (`DESK_FALLBACK_LINES`) are `desk_discipline` substitutions applied by `salience.filter_for_duty` to a line the model DID produce — phantom units, promises to leave the chair — not a failure fallback.]**; loop-guarded (NPC/unit/own traffic never answered); 10s cooldown **[STALE 2026-09-12: no 10s gate exists — the directed radio turn rides `LLM_DIRECTED_COOLDOWN = 4.0` (typeclasses/llm_npc.py) and `AnsweringFixture.ANSWER_COOLDOWN = 4.0` (typeclasses/items.py); `consider_radio_report` is deliberately "not gated by any answer cooldown".]**; availability count grounds the reply |
...
`radio/tune <frequency>` **[shipped as `tune <device> to <freq|scan>`; there is no `radio` command — same for §6.1's `radio <message>`, shipped as `transmit`/`xmit`/`xm`. Noted 2026-09-12]** — a walkie hears only its tuned band. **The dial
...
**[SUPERSEDED 2026-09-12 — the paragraph below is the pre-build problem statement, contradicted by this section's own ✅ SHIPPED banner. All three of its present-tense claims are false today: `_deliver` attaches `speech=` for hearing listeners plus `radio_frequency`/`radio_elected`; `LLMNpcMixin._hear_radio` (typeclasses/llm_npc.py) consumes it with the directed/elected/NPC-source/ambient gates; and the `radio` action tool → real `xmit` (with the `transmit_organ` no-handheld fallback) is the key-up path. Kept for the reasoning that produced §7.1–§7.6.]**
