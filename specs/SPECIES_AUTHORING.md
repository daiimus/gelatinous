...
> **CORRECTION (2026-09-12):** no longer two, and a species that skips the undocumented sites silently wears human prose. Beyond the two below and the two "optional secondary" places, four more locations carry species-keyed data. None of them are new — every one predates this guide's 2026-08-02 "verified" banner:
>
> - **`world/anatomy/organ_descriptions.py`** — `ORGAN_DESCRIPTIONS_BY_SPECIES` (`:265`), per-organ *condition prose* for non-organic bodies (robot, synth). Consulted before `ORGAN_DISPLAY` by `get_organ_default_description(organ, condition, species)` (`world/anatomy/organs.py:419`). Landed 2026-06-26, PR #845. (#2262 / PR #2275, 2026-08-24, is often miscited for this — that change routed the surgical *chart* through `get_organ_display_name` and filled one missing component description; it neither created the module nor the species lookup.)
> - **`world/medical/medical_messages.py`** — `BLEEDING_ROOM_BY_SPECIES` (`:20`), `DEATH_CAUSE_TEMPLATES_BY_SPECIES` (`:78`), `_GENERIC_DEATH_BY_SPECIES` (`:130`); #375, 2026-06-04. All four registered species are populated; an unpopulated species gets human death and bleeding prose.
> - **`world/mob_flavor/__init__.py`** — `_SHORT_DESCS_BY_SPECIES` (`:50`), `_LOOK_PLACES_BY_SPECIES` (`:57`), `_LONGDESCS_BY_SPECIES` (`:64`); #372, 2026-06-04. All four populated.
> - **`world/combat/messages/unarmed_<species>.py`** — `get_combat_message` prefers a species-suffixed module over the human-anchored `unarmed.py` (`world/combat/messages/__init__.py:81-85`); #365, 2026-06-04. `unarmed_rat.py` is the shipped example.
...
- **`world/medical/wounds/messages/*.py`** — per-injury-type wound prose. The existing tables are species-agnostic (humans share them). **Species packs (shipped 2026-07-05):** a species whose wounds must not read as human flesh registers a COMPLETE pack module in `messages.SPECIES_PACKS` (`robot.py` — torn plating, amber hydraulic weep, weld-seam scars; `synth.py` — cobalt blood, pearlescent seam scars). A pack species renders ALL surface-wound prose (every stage, plus `DESTROYED_BY_LOCATION` and `COMPOUND_DESCRIPTIONS`) from its pack — it never falls through to the shared flesh tables, which is the no-leak guarantee (pin new packs with a completeness test like `test_species_wounds.py`). Pack prose should hardcode the species `blood_color` (`world/anatomy/species.py`) and match the register of its `organ_descriptions.py` entries so surface and organ damage read as one body. **NOTE (2026-07-13, #1206):** WEAPON + severance message prose no longer hardcodes colour — it uses `{blood}`/`{Blood}` format fields resolved from the TARGET's species (human crimson / synth cobalt / robot amber) in `get_combat_message` + `get_severance_message`; a validator (`test_blood_tokens.py`) forbids bare colour words there. Pack wound tables (the species-keyed medical prose) still hardcode their own colour — that's the species speaking about ITSELF, not a weapon describing an arbitrary target. Humans and pack-less species (rat, ...) keep the shared modules unchanged. **Per-injury vocabulary (2026-09-14, #3512):** a pack may also carry `BY_INJURY = {injury_type: {stage: [...]}}` and `COMPOUND_BY_INJURY`; the lookup prefers them and falls back to the pack's stage-only tables, so a blow, a bullet and a blade read differently on a synth or a robot (a synth pools cobalt under the dermis; a robot dents) and an unauthored type still never leaks flesh prose. Compound lines keep `{severity}` on ONE wound and spend `{others_phrase}` on the rest; single-wound lines never use `{others_phrase}`; robot lines never use `{skintone}`. `test_species_wounds.PacksAnswerByInjuryType` pins all of it.
...
> **CORRECTION (2026-09-12):** two errors in the paragraph above.
>
> 1. **The fallback is per-SPECIES, not per-KEY.** `_resolve_species` (`world/anatomy/species.py:1441-1450`) substitutes the whole human entry only for an unknown or `None` species *name*. A REGISTERED species that omits a key gets an EMPTY collection from the getter — `frozenset(spec.get("severable_containers") or frozenset())` (`:1562-1580`), and the same `or {}` / `or ()` shape in `get_species_pair_keys`, `get_species_organs`, `get_species_grasping_containers`. So the real failure mode of a missing table is *absent anatomy and silent prose*, not human prose. (Species built by `_derive_*` deepcopy do inherit human values for anything they don't override — that path is the exception.)
> 2. **The list below is incomplete.** Every registered species also declares `grasping_containers` — the Mr. Hands slot set (#307 / PR-H1), declared at `species.py:370` (human) and `:983` (rat, empty), read by `get_species_grasping_containers` (`:1595`) and consumed in `typeclasses/characters.py:2031, 2306, 2358` and `world/combat/utils.py:313`; the rat's empty set is why a rat can't wield. Non-flesh bodies additionally declare `infection_immune` (synth `:1197`, robot `:1304`; getter `:1467`) and `organ_display` (synth `:1254`, robot `:1359`). `blood_color` is declared by **human too** (`:77`, alongside synth `:1191` / robot `:1300`; getter `:1457`) — the rat is the species that omits it and silently inherits crimson from `_DEFAULT_BLOOD_COLOR` (`:1454`), which is the trap for any new species whose fluid is not red. `body_capacities` and `longdesc_flex_nouns` are required too — they are documented under "Migrated follow-ups" below rather than in this table.
...
- `ORGAN_DISPLAY` (`world/anatomy/organs.py`). Per-organ display name and condition prose for autopsy / harvest rendering. Currently human-anchored. **SUPERSEDED (2026-09-12):** no longer the only remaining global, and no longer human-anchored on either axis — and both overrides shipped 2026-06-26, i.e. this follow-up register was already out of date when this guide was last verified on 2026-08-02. *Display names:* `organ_display` ships for BOTH `synthetic_humanoid` (the wetware register described below, `species.py:1254`) and `robot` — a full MECHANICAL register (brain→processor core, heart→power core, eyes→optical sensors, limb bones→struts/servos) at `species.py:1359`, resolved by `get_organ_display_name(organ, species)` (`organs.py:320`); issue #842 / PR #843. *Condition prose:* `world/anatomy/organ_descriptions.py` carries `ORGAN_DESCRIPTIONS_BY_SPECIES` banks for robot and synth (`:265`), consulted first by `get_organ_default_description(organ, condition, species)` (`organs.py:419`); PR #845. `ORGAN_DISPLAY` is now the shared ORGANIC table and the fallback for unoverridden organs — a new non-flesh species overrides at BOTH sites, not one. Rats fall through gracefully — shared organ names (brain / heart / left_lung / liver / etc.) get human prose which reads OK on a smaller body, and rat-specific bones (`left_foreleg_bone` / `tail_vertebrae`) get an underscore-stripped display name and empty prose (the renderer drops empty prose cleanly). **SHIPPED for synth (2026-07-13, #1204):** `SPECIES_DEFINITIONS[synthetic_humanoid]["organ_display"]` + `get_organ_display_name(organ, species)` — the WETWARE register (`_SYNTH_WETWARE` in `species.py`): soft core renamed to engineered-tissue nouns (brain→wetcore, heart→vat-heart, lungs→culture-lungs, liver→filter gland, kidneys→flush glands, stomach→culture gut) while bones/senses/form-visible organs stay "synthetic <organ>"; decay tiers compose free ("inert wetcore"); a test forbids industrial vocabulary (the rejected "cardiac pump" register — a Synthetic Humanoid is a replicant, not a machine). Humans untouched. An alien with non-mammalian systems would follow the same pattern.
