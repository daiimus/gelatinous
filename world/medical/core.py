...
class MedicalState:
    ...
    def organ_is_gone(self, name) -> bool:
        """Has the organ in slot *name* been taken out of this body?

        The one question every healing path asks (#3651). Organ loss is
        permanent (#3400): a harvested organ is ABSENT, and nothing but a
        transplant fills its slot again. `full_heal` asked this inline;
        wound dressing (`treatments.damaged_organs_at_location`) and the
        medical script's per-tick healing (`script._healing_organs`) did
        not ask it at all, so a dressing on the open abdomen a harvest
        leaves behind -- or an organ-repair item through the same incision
        -- healed a harvested kidney back to function.

        Two markers, either is enough:

        * the organ's ``injury_type == "harvested"``, stamped by
          `_mark_organ_removed` on a living target. `suture` moves the
          wound stage on and leaves this alone, so it is the durable
          one (#2455);
        * the name in the character's ``db.removed_organs`` record, which
          survives anything that rewrites the organ row (#3047).

        An INSTALL clears both, because a surgeon has put an organ back in
        that slot (`procedures._clear_removed_record`). A severed stump is a
        different absence with its own rules and is not this question.
        """
