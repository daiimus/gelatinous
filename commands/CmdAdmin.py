...
class CmdMigrateIdentity(Command):
    """
    Assign default identity attributes to existing characters.

    Usage:
        @migrate_identity          - Dry run (report only)
        @migrate_identity confirm  - Apply defaults to characters missing identity attrs

    Sets random height/build and gender-appropriate default sdesc_keyword
    for any character that lacks these attributes. Hair is left as None
    (bald) since we can't guess. sleeve_uid is generated if missing.

    This is a one-time migration for characters created before the
    ...
    def func(self):
        ...
        caller = self.caller
        dry_run = self.args.strip().lower() != "confirm"
        ...
        if dry_run:
            caller.msg("|yDry run mode.|n Pass 'confirm' to apply changes.")
            caller.msg("")
        ...
        for char in all_chars:
            try:
                ...
                # sleeve_uid
                if char.sleeve_uid is None:
                    needs_update = True
                    changes.append("sleeve_uid")
                    if not dry_run:
                        char.sleeve_uid = str(uuid.uuid4())

                # height
                if char.height is None:
                    needs_update = True
                    new_height = random.choice(HEIGHTS)
                    changes.append(f"height={new_height}")
                    if not dry_run:
                        char.height = new_height

                # build
                if char.build is None:
                    needs_update = True
                    new_build = random.choice(BUILDS)
                    changes.append(f"build={new_build}")
                    if not dry_run:
                        char.build = new_build

                # sdesc_keyword — set gender-appropriate default
                if char.sdesc_keyword is None:
                    needs_update = True
                    gender = char.gender
                    if gender == "male":
                        default_kw = "man"
                        ...
                        default_kw = "person"
                    changes.append(f"sdesc_keyword={default_kw}")
                    if not dry_run:
                        char.sdesc_keyword = default_kw

                # hair_color and hair_style are intentionally NOT set
                # (left as None = bald, which is a safe default)

                if needs_update:
                    migrated += 1
                    caller.msg(f"  {'[DRY]' if dry_run else '[SET]'} {char.key}: {', '.join(changes)}")
                else:
                    skipped += 1

            except Exception as e:
                errors += 1
                caller.msg(f"  |r[ERR] {char.key}: {e}|n")

        caller.msg("")
        caller.msg(f"|cMigration {'Report' if dry_run else 'Complete'}:|n")
        caller.msg(f"  {'Would migrate' if dry_run else 'Migrated'}: |w{migrated}|n")
        caller.msg(f"  Already OK: |g{skipped}|n")
        caller.msg(f"  Errors: |r{errors}|n")
