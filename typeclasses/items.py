...
class DispatchConsole(Radio):
    """The security base's dispatch console.

    A radio bolted to a desk. It receives on the emergency band, it can
    be switched off, and its roof antenna can be wrecked — that is the
    whole of it.

    It used to be the first ANSWERING device: it classified traffic,
    rolled security units, ran its own prompt and spoke with the
    operator's name on it. All of that has gone to the person in the
    chair (#2223, #2228). A colony operated by its people does not need
    its furniture to hold a second opinion, and an unmanned desk doing
    the job quietly was the worst of both — automation, just without the
    voice to give it away.

    What is left here is equipment. Whoever sits at it is the dispatcher
    (`world.radio.seated_base_station`); nobody sitting at it is nobody
    dispatching, and the colony finds out the hard way. That weakness is
    the setting, not a hole in it (owner ruling, 2026-08-22).
    """
