...
class Corpse(IdentityBearerMixin, Item):
    ...
    def worn_garments(self):
        """What the corpse is wearing: everything in its contents that
        declares coverage, filtered by `worn_at_death` where that record
        exists (#3107); no record means contents-wide, as the death
        transfer wrote it before the record existed. Distinct from
        `get_worn_items`, which is deliberately only the disguise-essential
        subset the identity signature consumes. Shared question with
        characters and severed parts (#3575).
        """
        worn_ids = getattr(self.db, "worn_at_death", None)
        out = []
        for item in self.contents:
            if getattr(item, "db", None) is None or not item.db.coverage:
                continue
